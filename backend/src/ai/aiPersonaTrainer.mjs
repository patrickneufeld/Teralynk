...
export async function trainPersonaModel(userId, fileMetadata, fileName) {
  try {
    logInfo(`[PersonaTrainer] Training initiated for user ${sanitize(userId)}`);
    ...
    logInfo(`[PersonaTrainer] Training complete for user ${sanitize(userId)}`, { resolvedPersona });
    return resolvedPersona;
  } catch (err) {
    logError(`[PersonaTrainer] Error training persona for user ${sanitize(userId)}`, err);
    throw err;
  }
}
