...
const saveWorkflowVersion = async (workflowId, changes) => {
    try {
        ...
        console.error('Error saving workflow version:', error.stack);
...
const getWorkflowVersionHistory = async (workflowId) => {
    try {
        const result = await query('SELECT * FROM workflow_versions WHERE workflow_id = $1 ORDER BY created_at DESC', [
            workflowId,
        ]);
        return result.rows;
    } catch (error) {
        console.error(`Error fetching version history for workflow ${workflowId}:`, error.stack);
