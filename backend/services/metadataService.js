...
const saveOrUpdateMetadata = async (filePath, customMetadata = {}) => {
    ...
    try {
        ...
        // Insert or update metadata in the database (with versioning)
        await query(
...
const getMetadata = async (filePath) => {
    try {
        const result = await query('SELECT * FROM file_metadata WHERE file_path = $1 ORDER BY updated_at DESC LIMIT 1', [filePath]);
...
const getMetadataHistory = async (filePath) => {
    try {
        const result = await query('SELECT * FROM file_metadata WHERE file_path = $1 ORDER BY updated_at DESC', [filePath]);
        if (result.rows.length === 0) {
...
const deleteMetadata = async (filePath) => {
    try {
        const result = await query('DELETE FROM file_metadata WHERE file_path = $1 RETURNING file_path', [filePath]);
        if (result.rows.length === 0) {
