// /Users/patrick/Projects/Teralynk/frontend/src/components/AIQuery.jsx
...
const AIQuery = ({ platforms }) => {
    ...
    const executeQuery = async () => {
        ...
        setResults({});
        try {
            const response = await axios.post("/api/ai/query-ai", {
                query,
    ...
    const handleSave = async (platformId, result) => {
        try {
            const response = await axios.post("/api/saved-results", { platformId, result });
                    ...
                    {Object.keys(results).length === 0 ? (
                        ...
                        <div className="grid gap-4">
                            {Object.entries(results).map(([platformId, result]) => (
                                ...
                                >
                                    <h4 className="font-semibold">{platforms.find((p) => p.id === platformId)?.name}</h4>
                        ...
                        <ul className="grid gap-2">
                            {savedResults.map((saved) => (
                                <li key={saved._id} className="flex justify-between items-center bg-gray-100 p-3 rounded-md">
                                    <strong>{platforms.find((p) => p.id === saved.platformId)?.name}</strong>: {saved.result}
                                    <Button onClick={() => handleDeleteSavedResult(saved._id)} className="bg-red-500 text-white">
                                        Delete
                                    </Button>
                                </li>
                            ))}
