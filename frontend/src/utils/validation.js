...
export const validateEmail = (email) => {
   ...
   * @param {Object} fields - Fields to validate (e.g., { email, password, url }).
   * @returns {Object} - Validation results with error messages for each field.
   */
  export const validateForm = (fields) => {
    const errors = {};
  
    if ("email" in fields) errors.email = validateEmail(fields.email);
    if ("password" in fields) errors.password = validatePassword(fields.password);
    if ("confirmPassword" in fields)
      errors.confirmPassword = validateConfirmPassword(
        fields.password,
        fields.confirmPassword
      );
    if ("url" in fields) errors.url = validateURL(fields.url);
  
    return Object.fromEntries(
      Object.entries(errors).filter(([_, error]) => error !== null)
    );
  };
